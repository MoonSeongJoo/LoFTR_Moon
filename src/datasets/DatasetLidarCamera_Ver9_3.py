...
from src.datasets.utils import invert_pose, rotate_forward, quaternion_from_matrix ,rotate_back #, read_calib_file
...
#             return self.__getitem__(new_idx)
        ...
        R0 = mathutils.Euler((radians(img_rotation), 0, 0))
        T0 = mathutils.Vector((0., 0., 0.))
...
#         print(f' gt_uv shape = {gt_uv.shape}', end='gt_uv end \n')        
        ...
        R0 = mathutils.Quaternion(R0).to_matrix()
        R0.resize_4x4()
        T0 = mathutils.Matrix.Translation(T0)
        RT0 = T0 @ R0 # version change matutils * --> @ 
        
        R = mathutils.Quaternion(R).to_matrix()
        R.resize_4x4()
        T = mathutils.Matrix.Translation(T)
        RT1 = T @ R # version change matutils * --> @ 

        pc_rotated = rotate_back(pc_in, RT1) # Pc` = RT * Pc        
        T_0to1 = torch.tensor(np.matmul(RT1, np.linalg.inv(RT0)), dtype=torch.float)[:4, :4]  # (4, 4)
...
#         rgb_np_resized = img.resize((640, 192), Image.LANCZOS)
        gray_img = cv2.resize(img, (640,192), interpolation=cv2.INTER_LINEAR)
        gray_img = torch.from_numpy(gray_img).float()[None] / 255  # (h, w) -> (1, h, w) and normalized
        scale = torch.tensor([1280/640, 384/192], dtype=torch.float)
...
#         rgb_img = img.resize((1280, 384), Image.LANCZOS)
        ...
        dense_depth_img_np_resized = torch.from_numpy(dense_depth_img_np_resized).float()[None] / 255  # (h, w) -> (1, h, w) and normalized
...
#         print ("-------corrs_max---------" , np.max(corrs[:,3]))
        ...
        if self.split == 'test':
            sample = {'image0': gray_img, 'depth0': gt_uv, 'image1' : dense_depth_img_np_resized, 'depth1': uv ,
                      'T_0to1' : T_0to1 , 'T_1to0' : T_1to0 ,
                      'K0': calib, 'K1' : calib , 'scale0' : scale , 'scale1' : scale,
                      'dataset_name': 'kitti', 
                      'point_cloud': pc_in, 'pc_rotated' : pc_rotated ,
                      'tr_error': T_torch, 'rot_error': R_torch, 'seq': int(seq), 'img_path': img_path,
                      'rgb_name': rgb_name + '.png', 'item': item, 'extrin': RT_torch,
                      'initial_RT': initial_RT , 'corrs' : corrs ,  'lidar_gt' : input_lidar_gt_pytorch,
                      }
        else:
            sample = {'image0': gray_img, 'depth0': gt_uv, 'image1' : dense_depth_img_np_resized, 'depth1': uv ,
                      'T_0to1' : T_0to1 , 'T_1to0' : T_1to0 ,
                      ...
                      'corrs' : corrs ,  'lidar_gt' : input_lidar_gt_pytorch,
                      }
